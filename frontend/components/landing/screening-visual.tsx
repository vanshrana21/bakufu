...
export function ScreeningVisual() {
  ...
  return <svg viewBox="0 0 720 710" role="img" aria-labelledby={`${id}-title ${id}-desc`}>
    ...
    {sites.map(({x,y})=><path data-screen-inbound pathLength="1" key={`${x}-${y}`} d={`M${x} ${y+24}V${y+166}`} fill="none" stroke="var(--screen-steel)" strokeOpacity="0.7"/>)}
